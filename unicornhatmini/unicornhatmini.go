...
// Package unicornhatmini implements interfacing code to Pimoroni's Unicorn HD hat.
...
// Dev represents a Unicorn HAT Mini (https://shop.pimoroni.com/products/unicorn-hat-mini)
...
// New returns a unicornHATMini driver that communicates over SPI.
//
...
func (device *Dev) String() string {
	return fmt.Sprintf("UnicornHATMini{%d, %d}", width, height)
}
